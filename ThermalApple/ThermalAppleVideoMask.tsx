...
// Helper to load video from asset bundle
...
export function ThermalApple({
  ...
  const { currentFrame } = useVideoFromAsset(
    require("./assets/largetall_2x.mp4"),
    {
      looping: true,
    }
  );
  ...
  const currentTouchHeat = useSharedValue(0);
  ...
  const thermalShader = Skia.RuntimeEffect.Make(`
    ...
    half4 main(float2 coord) {
      ...
      // Sample video texture for base temperature - bright pixels = hot zones, dark pixels = cold zones  
      half4 videoSample = videoFrame.eval(uv * uResolution);
      float videoHeat = dot(videoSample.rgb, float3(0.299, 0.587, 0.114)); // Convert to luminance
      float map = videoHeat;
  ...
  useFrameCallback((frameInfo) => {
    ...
    if (isActive.value > 0.5) {
      currentTouchHeat.value = Math.min(currentTouchHeat.value + 0.02, 1);
    } else {
  ...
  return (
    <GestureDetector gesture={gesture}>
      ...
      >
        {logoImage && currentFrame && thermalShader && (
          <Fill>
